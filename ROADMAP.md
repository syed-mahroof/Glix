...
**Last Updated:** 2026-08-03 (Phase 76 — investigated a user report that Discover, Profile, and "most things" stopped working right after Phase 75's EAS update + backend redeploy. Found no code or backend regression after extensive verification (`git diff` confirmed Discover's files untouched this phase; `tsc` clean; the real production backend end-to-end tested clean against both a fresh and a fully populated test account across every Phase 75 endpoint). Root cause is most likely Render's free-tier cold-start window coinciding with the fresh redeploy, compounded by a real gap: the returning-user boot path skips the cold-start-aware wait the login path already has. Shipped a "waking up the server" hint on Discover/Community's loading states and the app's first-ever error boundary (confirmed completely absent before this). See Phase 75 below.)
