...
**Last Updated:** 2026-08-21 (Phase 85 — 10-item user bundle, ALL FIVE BATCHES NOW LANDED. Batch E closes it out: no roadmap items added or completed (accessibility fixes + tests, not feature work). Scoped the "design polish across 10 screens" ask down to its objective half after a checklist audit found the codebase already solid — fixed the 20 real gaps it found (14 icon buttons with no accessibilityLabel, 6 touch targets under 44pt), and explained why the subjective spacing/hierarchy pass needs a screenshot feedback loop this session didn't have rather than delivering it blind. Added 13 backend + 20 client regression tests, one per Batch A-D mechanism that lacked dedicated coverage. Published a 34-item manual QA checklist as an artifact. Backend suite 228→241, client jest 49→69, `tsc --noEmit` clean. See `PROJECT_STATUS.md`'s Phase 85 (Batch E) section and `AUDIT.md`'s matching entry. See Phase 85 (Batch A-D) below.)
**Last Updated (prior):** 2026-08-21 (Phase 85 cont., Batches C+D of the same 10-item user bundle. Batch C — new roadmap-adjacent capability: Movie Analytics gained a language breakdown, a "Watched in {year}" list, a monthly strip, a shortest/longest runtime pairing, and the user's own rating average/distribution, all from data already on hand (no migration). Batch D — measured the real production backend against the user's own real-library account; found and fixed `AnalyticsGenresView` at a flat ~2.1s/call (dead duplicate query + no caching, both fixed), confirmed two other candidates from the plan didn't need touching (already fast / false-positive grep hits), and flagged (without acting on, per the plan's own caution) `/watchlist/`'s 1.5MB payload as the real cost behind its response time. Backend suite 228/228, `tsc --noEmit`/`jest` clean. See `PROJECT_STATUS.md`'s Phase 85 (Batch C/D) sections and `AUDIT.md`'s matching entries. Batch E still to come. See Phase 85 (Batch A/B) below.)
**Last Updated (prior):** 2026-08-21 (Phase 85 cont., Batch B of the same 10-item user bundle — no roadmap items added or completed (UI fix + redesign, not new feature work). Discover Hub grid-gap fix (uniform card height for FlashList v2's `numColumns` grid, plus a pagination dedupe fix), and a full-bleed hero + glassmorphic header redesign (hero now renders behind the floating header instead of below a hardcoded gap; search bar/filter button upgraded to real `GlassBlur` + `glassFillStrong` blur, matching the recipe already used elsewhere in the app). `tsc --noEmit` clean, `jest` 49/49, no backend changes. See `PROJECT_STATUS.md`'s Phase 85 (Batch B) section and `AUDIT.md`'s matching entry. Batches C/D/E of the same bundle still to come. See Phase 85 (Batch A) below.)
**Last Updated (prior):** 2026-08-21 (Phase 85, Batch A of a 10-item user bundle — 6 device-reported bugs fixed, no roadmap items added or completed (bug fixes, not feature work — see `PROJECT_STATUS.md`'s Phase 85 section and `AUDIT.md`'s matching entry for the full fix list). Fixed: Default Layout reset on relaunch (root cause beyond Phase 84's fix — the preference shared an AsyncStorage row with the whole watchlist; moved to its own store), a stale Catch-Up modal reappearing after a show already reads complete (client re-validation + a new server-side aired-date grace period closing a UTC-vs-local-timezone gap), Watch List showing a different weekday than Upcoming for the same episode (Watch List was formatting TMDB's raw network-local date instead of resolving the real air instant), a season-marking race that could show 0% watched right after marking episodes (`fetchWatchlist()` had no last-write-wins guard), and a show reading "Series Complete" + confetti while later untracked seasons remain (completion now requires full season coverage, not just a cached-episode ratio, plus a background task to backfill the missing season). Backend suite 222/222, `jest` 49/49, `tsc --noEmit` clean (pre-existing FlashList typing noise only). See Phase 84 below.)
**Last Updated (prior):** 2026-08-07 (Phase 84 — client-mobile bug/perf pass, no roadmap items added or completed (no new feature work — see `PROJECT_STATUS.md`'s Phase 84 section and `AUDIT.md`'s matching entry for the full fix list). Fixed a grid-mode mark-watched lag caused by an unawaited network call sitting in front of the store commit, a wire-race that fix exposed in `bulkToggleWatchState`, two silent-tap-swallow animation bugs in `ShowRow`/`MovieRow`, an intermittent Default Layout reset on relaunch (pre-hydration write racing zustand's rehydration, fixed across all four persisted stores), a `HeroCarousel` Rules-of-Hooks violation, and a general tab-switch/navigation responsiveness pass (`Intl.DateTimeFormat` caching, an optimistic tab-bar pill, four unnecessary FlashList remounts removed, a dead worklets dependency removed). `tsc --noEmit` clean, `jest` 47/47. See Phase 76 below.)
**Last Updated (prior):** 2026-08-03 (Phase 76 — root-caused and fixed a fatal Phase 75.8 regression that made the app freeze and get killed: `(tabs)/profile.tsx`'s new focus-refetch staleness guard fed itself, because `analyticsDirtyAt` was a `useCallback` dependency of the `useFocusEffect` while `fetchProfile()` bumps `analyticsDirtyAt` synchronously as its first statement — an unterminating focus→fetch→bump→re-run cycle firing 4 requests per turn. The resulting storm also backed up `lib/api.ts`'s per-request `SecureStore` interceptor, hanging other screens' requests *before* axios's timeout clock started — which is why Discover spun forever with no error. Fixed by making the flag non-reactive plus a 2s minimum-refetch floor; `analytics.tsx` hardened identically; every `useFocusEffect` in the repo swept. This supersedes the same phase's earlier "no code regression found" conclusion, which is retained in the docs rather than deleted. Also shipped: a "waking up the server" hint on Discover/Community's loading states, the app's first-ever error boundary, and the missing `profile/anime` `<Stack.Screen>` registration. `tsc --noEmit` clean, `jest` 12/12. See Phase 75 below.)
**Last Updated (prior):** 2026-08-03 (Phase 75 — 9-phase user-approved plan in one continuous pass: real per-episode air times via TVmaze `airstamp` (migration `0017`), a first-class Shows/Anime split + "My Anime" profile screen, reviews gated on watched, context-aware Lists defaults, a default-layout preference system (global + per-screen override), Community's Activity feed switched from watch history to real reviews (+ a self-follow-button-400s fix + Discussions 8→1 request collapse), a rewatch system (`ShowRewatch`/`RewatchEpisodeState`/`MovieRewatch`, migration `0018`, parallel to `WatchState` by design), a Celery/Redis cost pass (gossip/mingle/heartbeat off, throttling off Redis, wider polling) plus 5 cartesian-join query fixes and 2 N+1 fixes, and a closing sweep (shared `FilterPill`, dead-code removal). Plus a user-reported auto-logout bug fixed via two independent root causes in the token-refresh/auth-boot path, outside the plan's scope. Backend suite 148→192 (+44), `tsc --noEmit` clean, `jest` 12/12, `manage.py check` clean, two new additive migrations applied. Render redeploy (for the infra savings to take effect) not yet triggered. See Phase 74 below.)
...
**Last Updated (prior):** 2026-07-28 (Phase 69 — user-reported bundle: 9 items across Upcoming's Overdue section (real recency bound this time, gated on the episode's own air date not the show's activity), both home-screen widgets (headless render fix verified against the native module's own source, multi-item day grouping, flat black redesign, resize-envelope widening), Render cold-start UX (new `/health/` endpoint + client warmup poll + real error/retry state — previously `ready` flipped true unconditionally even on total failure), detail-screen action buttons (found correctly wired, added real busy-state feedback), My Movies "Last Watched" (real filter-bucket fix), Discover/Watchlist filter sheets (Trending highlight reverted to normal pill behavior, defensive scroll padding, new `glassFillStrong` opacity token), and new Trailer + Share features on both detail screens — plus a backend perf fix (TMDB `requests.Session` now shared per-process instead of rebuilt on every call). `tsc --noEmit` zero errors, backend trio clean/65-65 (run inside the container after finding the host venv's Postgres was hitting a stray native Windows service on the same port). Full detail in `context.md`/`AUDIT.md`'s Phase 69 entries. Also resolved a previously-open Remaining/Future Work item: `achievements.tsx`/`year-review.tsx` turned out to be root-Stack routes that remount fresh every visit, not sharing Profile/Analytics' frozen-tab staleness class after all — removed from the table below. See Phase 68 below.)
**Last Updated (prior):** 2026-07-24 (Phase 68 — Batch 3 closing pass: cross-checked every reported bug against the phase list (see `AUDIT.md`'s Phase 68 table), corrected the Phase 57/58/60 table rows and their `PROJECT_STATUS.md`/`AUDIT.md` sections to reflect that Batch 2's own "Fixed" claims for them didn't hold on-device, and re-confirmed via a bootstrap pass that Phases 54/55/56/59/61 genuinely do hold. No new bugs found beyond what the batch's own bootstrap/Phase 63-67 work already covered. `tsc --noEmit` stayed at zero errors (first achieved Phase 62) through every phase this batch — full detail in `AUDIT.md`/`PROJECT_STATUS.md`. See Phase 67 below (completion celebration, new feature), then 66 (movies remove/add — investigated, doesn't reproduce, no code change), 65 (profile stats — real fix this time), 64 (Discover filter sheet — real fix this time), 63 (My Shows/My Movies filter sheet — full rework).)
...
| 54 | Upcoming Tab Flood Bounded — overdue items now gated to started-and-recently-behind shows (14-day window, reused from `buildRows()`), collapsible "OVERDUE (N)" section defaulting closed | 2026-07-24 ✅ |
| 55 | Widget Flood Fix + Redesign — `syncWidgetData()` now excludes Overdue items from the widget entirely (strictly-future only), Upcoming widget rows redesigned with a bigger right-aligned day-count badge + glassmorphism card styling (Android + iOS) | 2026-07-24 ✅ |
| 56 | Watch History Live Sync — `toggleWatchState()`/`bulkToggleWatchState()` now update the Watch History tab optimistically (prepend/filter, snapshot-and-revert on error), matching the existing watchlist convention; audited every other Shows Hub tab, all already correct | 2026-07-24 ✅ |
| 57 | My Shows/My Movies Filter Overhaul — pill row replaced with a floating "FILTERS" button + bottom sheet (`WatchlistFilterSheet.tsx`), fixing the pill-row height regression; "Up to Date" filter replaced with a "Last Watched" sort on both screens; new "Animation" tag filter for Movies | 2026-07-24 ⚠️→✅ redone Phase 63 (shipped rendering inline behind the list — mount-order bug) |
| 58 | Filter Sheet Scroll-Clipping + Full Toggle-Off — fixed `DiscoverFilterSheet`/`WatchlistFilterSheet`/`LanguageFilterModal` all clipping tail content past their fixed/max height with no scroll; every single-select filter pill (incl. "Trending", Status) now supports toggle-off; genre/language/anime/pagination re-verified clean | 2026-07-24 ⚠️→✅ redone Phase 64 (safe-area padding + Trending highlight-by-default bug) |
| 59 | Rating/Review Save Confirmation + Star Animation — `onSaved` callback wired to a new screen-level Snackbar ("Rating saved"/"Note saved"/"Review deleted"), explicit "Save note" button, privacy caption moved out of the placeholder, per-star Reanimated pop animation | 2026-07-24 ✅ |
| 60 | Profile Stats Sync Correctness — audited every `total_time_watched` mutation path (clean), added `recalculate_total_time_watched`/`recalculate_streak_action` admin recovery tools, fixed Analytics dashboard/streak/heatmap staleness on Profile-tab refocus (`useFocusEffect`) | 2026-07-24 ⚠️→✅ this audit was real but targeted the wrong screen — redone Phase 65 for the actual Profile Hub tab |
| 61 | Widget Reliability (Offline/Empty) — audited every `syncWidgetData()` call site (all correctly success-gated), re-confirmed Android SharedPreferences cache + `WidgetProvider.tsx`/iOS WidgetKit snapshot offline resilience, fixed widgets showing "empty" instead of "log in"/"syncing" after logout or before first sync (`loggedOut` marker) | 2026-07-24 ✅ |
| 62 | Final Closing Pass — closed both standing `tsc --noEmit` baseline errors (zero errors for the first time this engagement), full backend/grep/git-status verification, deferred item consolidated into Remaining/Future Work | 2026-07-24 ✅ |
| 63 | My Shows/My Movies Filter Sheet Rework — fixed real mount-order bug (sheet painted behind the list despite correct `position: absolute`), status tabs restored to a top pill row, sheet trimmed to Language + tags only, `FloatingFilterButton` rebuilt bottom-centered/safe-area-aware/always-accent | 2026-07-24 ✅ |
| 64 | Discover Filter Sheet Real Fix — safe-area-aware bottom padding (was clipped under gesture-nav insets), Trending sort pill no longer renders as falsely "active" by default, always-visible scrollbar | 2026-07-24 ✅ |
| 65 | Profile Hub Watch-Time Stats Real Fix — Months/Days/Hours/Mins now derived client-side from `total_time_watched` (backend never returned the 3 derived fields in any toggle response, so they were permanently frozen), `useFocusEffect` added to the actual Profile tab | 2026-07-24 ✅ |
| 66 | Movies Remove/Add-to-Watchlist — investigated, does not reproduce; appears already fixed by Phase 46, predating this batch's bug list | 2026-07-24 🟢 no code change |
| 67 | Completion Celebration — confetti + non-blocking banner when a show's final episode is marked watched and the show has fully ended, trigger computed from the toggle action's own before/after state (never a refetch), Reanimated-only | 2026-07-24 ✅ |
| 68 | Batch 3 Closing Pass — cross-checked every reported bug against the phase list, corrected Phase 57/58/60's "Fixed" claims in place, re-confirmed Phases 54/55/56/59/61 genuinely hold, full quality/performance re-check | 2026-07-24 ✅ |
| 69 | User-Reported Bundle — Overdue real recency bound (episode air date, not show activity), widget headless-render fix + multi-item day grouping + flat redesign + resize-envelope widening, Render cold-start health-check/warmup/retry UX, detail-screen action-button busy states, My Movies Last Watched real fix, Discover Trending highlight reverted + defensive scroll padding, Watchlist sheet opacity token, new Trailer + Share features, shared TMDB session perf fix | 2026-07-28 ✅ |
| 70 | Multi-User Performance & Stability + Add-to-List — gunicorn gthread concurrency + `max-requests` recycling, short-TTL per-user response cache with signal-based invalidation for Watchlist/Movie endpoints, stats resync endpoint + tap-to-sync UI, Android widget countdown parity fix, Movies Hub "Last Watched" pill, new user-created Lists feature (models/API/store/screens/sheet), 4 confirmed bug fixes (AnalyticsAchievementsView 500, duplicate query, dead permission class, dead local var), Jest AsyncStorage mock fix | 2026-07-29 ✅ |
| 71 | Achievements Silent-Failure Fix + Recommendations/Digest/Share — shared `ErrorState.tsx` wired into all 4 analytics screens (failed fetch no longer looks like "0 badges earned"), shareable Year in Review image export (`react-native-view-shot` + `expo-sharing`), content-rich weekly push digest (movies + top show + upcoming count), real cross-library "For You" recommendations (`GET /api/recommendations/for-you/`, new Discover row) | 2026-07-29 ✅ |
...
| 76 | Post-Update "Everything Blank" Investigation — user reported Discover/Profile/most tabs broken right after Phase 75's EAS update + backend redeploy; exhaustive investigation (git diff, `tsc`, live production backend testing against fresh + fully populated accounts across every Phase 75 endpoint) found no code or backend regression. Most plausible cause: Render free-tier cold-start window coinciding with the fresh redeploy, compounded by the returning-user boot path skipping the cold-start-aware wait the login path already has. Shipped `useColdStartHint()` "waking up the server" messaging on Discover/Community, plus the app's first-ever `ErrorBoundary` (confirmed completely absent before this) | 2026-08-03 ✅ |
...
- 🔴 **Phase 73 correction — three real bugs found in this "done" feature, all fixed:** (1) the new-episode alert's "was this episode already known" check ran *before* that day's re-fetch, but the 6-hourly `sync_active_shows` sweep plus TMDB publishing episodes weeks ahead meant an episode was *always* already cached by the time its air date arrived — the condition could essentially never be true in practice, not just in an edge case. Fixed via a new `CachedEpisode.notified_at` gate. (2) a token registered at cold-start only — a fresh login/register never called `registerForPushNotificationsAsync()` at all (`loading.tsx`, the actual post-auth boot screen, never did), so a brand-new user's push token was never set until an incidental force-quit-and-relaunch. Fixed by registering from `loading.tsx` too. (3) `lib/notifications.ts`'s foreground handler only set the now-deprecated `shouldShowAlert` field; the installed `expo-notifications` version needs `shouldShowBanner`/`shouldShowList` to actually display anything while the app is open. Fixed by adding both. See `AUDIT.md`'s Phase 73 entry for the full table.
...
- ✅ FlashList `numColumns`/`extraData`/keyed-remount handling per screen to avoid layout thrashing when switching layouts — **correction, Phase 84 (2026-08-07):** the "keyed-remount" half of this turned out to be unnecessary and was removed. Verified directly against `@shopify/flash-list`'s `RecyclerViewManager.js` source that a `numColumns` change is handled by `updateLayoutParams()` on the existing layout-manager instance (only `horizontal` toggling is unsupported without a remount) — the `key={...-${layout}}` props at all 7 of these call sites were forcing a full FlashList unmount+remount (destroying every visible row/card, expo-image instance, and Reanimated shared value) on every list/grid toggle for no correctness reason; `extraData`, already present at every site, was already sufficient on its own. See `AUDIT.md`'s Phase 84 entry.
...
### Import/Export
- ✅ POST `/api/import/tvtime/` — enqueues an `ImportJob`, returns `202 {job_id, total, status}`. **Phase 72:** resume-match against a `FAILED` job now keyed on a stored `payload_fingerprint` (sha256, computed once at creation) instead of deep-comparing the full multi-MB `payload` on every retry request
- ✅ GET `/api/import/status/<job_id>/` — progress + final counts, polled by the client
- ✅ `core/tasks.py::run_tvtime_import` — Celery task; resolves tvdb/imdb → TMDB via `/find/`, preserves `watched_at`, bulk-creates `WatchState`. **Phase 72:** `TMDBService.find_by_external_id()` now cached 30 days (was live-hit on every item, every submission); `get_season_episodes()` writes a season via one `bulk_create(update_conflicts=True)` instead of a SELECT+UPSERT per episode
- ✅ `core/tasks.py::resume_stalled_imports` — **NEW Phase 72** — Celery beat task (every 5 min) that re-enqueues a `PENDING`/`RUNNING` `ImportJob` whose `updated_at` has gone quiet for 6+ minutes, self-healing a job orphaned by a mid-chunk container restart instead of leaving it stuck forever
...
- ✅ `find_by_external_id()`
- ✅ `get_external_ids()` — **NEW Phase 73** — `/tv/{id}/external_ids`, cached 30 days; feeds `core/airtime.py`'s TVmaze lookup (TMDB itself has no time-of-day data at all)
...
- ✅ ErrorBoundary — **NEW Phase 76** — the app's first render-crash safety net; wraps the root `<Stack>` in `_layout.tsx`, themed "Something went wrong / Try again" fallback
...
- ✅ Free-tier resource hardening — **NEW Phase 72** — `GZipMiddleware` (every response is JSON), `DATA_UPLOAD_MAX_MEMORY_SIZE` raised 2.5MB→25MB (a large TV Time export's body exceeded Django's default), `CELERY_TASK_ACKS_LATE`/`CELERY_WORKER_PREFETCH_MULTIPLIER=1`/`CELERY_WORKER_MAX_TASKS_PER_CHILD=40` — render-start.sh runs gunicorn + celery worker + beat in one free-tier container, and an unbounded prefetch or a lost-on-restart unacked task were both real contributors to the reported "Instance failed"/502 events during a big import
...
| `app/_layout.tsx`'s returning-user boot path (token already in SecureStore) never runs the cold-start-aware `waitForBackend()` wait/message loop that `app/loading.tsx` already has for login/register — found Phase 76 while investigating a post-redeploy "everything blank" report; deliberately not fixed this phase (would add a wait to every cold app open, not just right after a redeploy) in favor of the narrower `useColdStartHint()` messaging on Discover/Community | Medium |
...
| `analytics.tsx` (6 endpoints) / `profile.tsx` (3 endpoints, 2 heavy) refetch unconditionally on every focus rather than gating on staleness — flagged Phase 74 as a real perf item; **Phase 75 added a `analyticsDirtyAt` staleness guard to both**, and **Phase 76 fixed that guard's fatal self-triggering loop on `profile.tsx`** (see AUDIT.md Phase 76) | Resolved (Phase 75, corrected Phase 76) |
| No test harness exists for hook/screen behavior — jest has no `@testing-library/react-native` and no navigation harness, so `useFocusEffect` interactions like Phase 76's refetch loop cannot be regression-tested and are covered only by code inspection. Adding one would have caught this class of bug | Medium |
