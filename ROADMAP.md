...
| Widgets | 95% (data bridge + config wiring fixed 2026-07-14; Batch 2's Phases 54/55/61 fixed upcoming-list flooding/scoping, visual redesign, and logged-out/never-synced empty-state copy; both platforms still need an EAS build for on-device verification — the one remaining gap) |
| Infrastructure | 100% |
...
| `achievements.tsx`/`year-review.tsx`: same mount-only fetch staleness Phase 56/60 fixed elsewhere (Watch History/Analytics tabs) | Medium |
