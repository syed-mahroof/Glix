...
| 57 | My Shows/My Movies Filter Overhaul — pill row replaced with a floating "FILTERS" button + bottom sheet (`WatchlistFilterSheet.tsx`), fixing the pill-row height regression; "Up to Date" filter replaced with a "Last Watched" sort on both screens; new "Animation" tag filter for Movies | 2026-07-24 ✅ |
| 58 | Filter Sheet Scroll-Clipping + Full Toggle-Off — fixed `DiscoverFilterSheet`/`WatchlistFilterSheet`/`LanguageFilterModal` all clipping tail content past their fixed/max height with no scroll; every single-select filter pill (incl. "Trending", Status) now supports toggle-off; genre/language/anime/pagination re-verified clean | 2026-07-24 ✅ |
...
| 60 | Profile Stats Sync Correctness — audited every `total_time_watched` mutation path (clean), added `recalculate_total_time_watched`/`recalculate_streak_action` admin recovery tools, fixed Analytics dashboard/streak/heatmap staleness on Profile-tab refocus (`useFocusEffect`) | 2026-07-24 ✅ |
